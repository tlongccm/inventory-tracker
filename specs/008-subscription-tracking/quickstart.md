...
## Update: Field Definition Changes (2025-12-22)

### Summary

The subscription field definitions have been updated based on `subscription_fields.csv`. This requires:
1. Database migration for new/changed fields
2. Model updates (SQLAlchemy)
3. Schema updates (Pydantic)
4. Frontend type and display header updates

### Field Changes

| Change | Field | Details |
|--------|-------|---------|
| RENAME | `authentication` → `google_authentication` | Now email type |
...
```bash
cd backend
alembic revision --autogenerate -m "update_subscription_fields_2025_12_22"
```

2. **Update model** (`backend/app/models/subscription.py`):
```python
# Change authentication to google_authentication
google_authentication = Column(String(200))  # was: authentication

# Rename cost to payment_amount and change to Numeric
payment_amount = Column(Numeric(12, 2))  # was: cost String(100)

# Change email_volume_per_week to Numeric
email_volume_per_week = Column(Numeric(10, 2))  # was: String(100)

# Add new fields
renewal_frequency = Column(SQLEnum(RenewalFrequency))
notes = Column(Text)

# Update PaymentFrequency enum
class PaymentFrequency(str, PyEnum):
    MONTHLY = "Monthly"
    YEARLY = "Yearly"    # was ANNUAL
    AD_HOC = "Ad Hoc"    # was OTHER

# Add RenewalFrequency enum
class RenewalFrequency(str, PyEnum):
    MONTHLY = "Monthly"
    YEARLY = "Yearly"
    AD_HOC = "Ad Hoc"
```

3. **Update schemas** (`backend/app/schemas/subscription.py`):
   - Rename `authentication` to `google_authentication`
   - Rename `cost` to `payment_amount` and change type to `Decimal`
   - Change `email_volume_per_week` type to `Decimal`
   - Add `renewal_frequency` field
   - Add `notes` field
   - Update `payment_frequency` enum values

4. **Update frontend types** (`frontend/src/types/subscription.ts`):
```typescript
interface Subscription {
  // Rename
  google_authentication?: string;  // was: authentication

  // Rename + Type changes
  payment_amount?: number;  // was: cost string
  email_volume_per_week?: number;  // was: string

  // New fields
  renewal_frequency?: 'Monthly' | 'Yearly' | 'Ad Hoc';
  notes?: string;

  // Updated enum
  payment_frequency?: 'Monthly' | 'Yearly' | 'Ad Hoc';  // was: Monthly/Annual/Other
}
```

5. **Update display headers** in list/form components:
...
Update CSV import to accept both old and new column names:
- `authentication` maps to `google_authentication`
- `Google Auth` maps to `google_authentication`

### Testing Checklist (Update)

- [ ] Migration runs without errors
- [ ] Existing data preserved (authentication → google_authentication)
- [ ] Cost values converted to numeric (NULL for non-parseable)
- [ ] Email volume values converted to numeric
- [ ] Payment frequency values updated (Annual → Yearly, Other → Ad Hoc)
- [ ] New fields (renewal_frequency, notes) accept input
- [ ] Display headers show correctly in list/form
- [ ] CSV import accepts both old and new column names
- [ ] CSV export uses new column names
