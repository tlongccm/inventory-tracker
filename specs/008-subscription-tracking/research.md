...
## Session 2025-12-22: Field Definition Update

### 7. Database Migration Strategy for Field Changes

**Question**: How to handle the `authentication` → `google_authentication` field rename and type changes (`cost` → `payment_amount`, `email_volume_per_week`)?

**Decision**: Use Alembic migration with data preservation

**Rationale**:
- Alembic migrations are already in use for this project (see `backend/alembic/versions/002_add_subscription_tables.py`)
- Single migration can handle: add new columns, copy data, drop old columns, rename
- SQLite (dev) and MySQL (prod) both support these operations

**Migration Strategy**:
1. Add new columns: `google_authentication`, `renewal_frequency`, `notes`, `payment_amount` (Numeric)
2. Add temporary column for type change: `email_volume_per_week_new` (Numeric)
3. Copy data: `authentication` → `google_authentication`, convert string cost → `payment_amount` numeric, email values to numeric
4. Drop old columns: `authentication`, `cost`, `email_volume_per_week`
5. Rename temporary column to final name

**Alternatives Considered**:
- Direct column modify: Rejected - SQLite doesn't support ALTER COLUMN for type changes
- Manual SQL: Rejected - Alembic provides version control and rollback

---

### 8. Handling Existing Data for Type Conversions

**Question**: How to convert existing string values in `cost` to `payment_amount` and `email_volume_per_week` to float?

**Decision**: Parse numeric values, set NULL for unparseable data

**Rationale**:
- Existing data may contain text descriptions like "$100/month" or "~5 emails"
- Numeric extraction where possible, NULL otherwise preserves data availability
...
**Alternatives Considered**:
- Strict parsing (fail on non-numeric): Rejected - would lose existing data
- Keep as string: Rejected - spec requires float for calculations

---

### 9. PaymentFrequency Enum Update

**Question**: Current enum has `Monthly/Annual/Other`. Spec requires `Monthly/Yearly/Ad Hoc`.

**Decision**: Update enum values to match spec

**Rationale**:
- "Yearly" and "Annual" are equivalent semantically
- "Ad Hoc" replaces "Other" for clarity
- Database stores string values, so migration updates existing values
...
### 10. Frontend Display Header Updates

**Question**: How to update column display headers without breaking existing functionality?

**Decision**: Update column configuration in frontend only

**Rationale**:
- Column headers are frontend presentation logic
- No backend changes needed for display names
- Existing column configuration handles this

**Files to Update**:
- `frontend/src/types/subscription.ts` - TypeScript types
- `frontend/src/components/SubscriptionList.tsx` - Column definitions
- `frontend/src/components/SubscriptionForm.tsx` - Form field labels
- `frontend/src/components/SubscriptionDetail.tsx` - Detail view labels

---

### 11. CSV Import/Export Field Mapping

**Question**: How to handle CSV import/export with changed field names?

**Decision**: Accept both old and new column names on import; export uses new names

**Rationale**:
- Backward compatibility for existing CSV exports
- New exports use canonical field names from spec
- CSV mapping already exists in subscription service

**Import Mapping** (backward compatibility):
```python
FIELD_ALIASES = {
    'authentication': 'google_authentication',
    'Authentication': 'google_authentication',
    'Google Auth': 'google_authentication',
}
```

---

## Field Change Summary (2025-12-22)
...
| UPDATE | `payment_frequency` enum | Annual→Yearly, Other→Ad Hoc |

**Ready for Phase 1**: All unknowns resolved.
